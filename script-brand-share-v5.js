...
(function () {
  ...
  function getShareUrl() {
    ...
    url.hash = '';
    url.searchParams.set('share', 'gold-v5');
    return url.toString();
  }

  function upgradeExistingShareControls() {
    ...
    document.querySelectorAll('a[href*="wa.me/"]').forEach((link) => {
      link.href = `https://wa.me/?text=${encodeURIComponent(`${title} - Daily Dose Devotions ${url}`)}`;
    });

    document.querySelectorAll('[data-share-platform="native"]').forEach((button) => {
      button.addEventListener('click', async () => {
        try {
          if (navigator.share) {
            await navigator.share({ title, text: `${title} - Daily Dose Devotions`, url });
          } else {
            await navigator.clipboard.writeText(url);
          }
        } catch {}
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { upgradeExistingShareControls(); addAfterArticle(); addToArchivePage(); });
  } else {
    upgradeExistingShareControls();
    addAfterArticle();
    addToArchivePage();
  }
})();
